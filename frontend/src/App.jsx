...
function ProductForm({ onSave, editingProduct, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={styles.form}>
      ...
      <input style={styles.input} name="price" type="number" step="0.01" placeholder="Precio" required value={form.price} onChange={handleChange} />
      <input style={styles.input} name="stock" type="number" placeholder="Stock" value={form.stock} onChange={handleChange} />
...
export default function App() {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '48px auto', padding: '20px', fontFamily: 'sans-serif' }}>
          ...
          <tbody>
            {products.map(p => (
              <tr key={p._id}>
                ...
                <td style={tdStyle}>
                  <button style={{ ...btnStyle, background: '#fd7e14', color: '#fff' }} onClick={() => setEditingProduct(p)}>Editar</button>
                  <button style={{ ...btnStyle, background: '#dc3545', color: '#fff' }} onClick={() => handleDelete(p._id)}>Eliminar</button>
                </td>
              </tr>
            ))}
          </tbody>
